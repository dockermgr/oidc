...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202410081213-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Tuesday, Oct 08, 2024 12:13 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for oidc
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
export APPNAME="oidc"
export VERSION="202410081213-git"
...
DOCKER_REGISTRY_REPO_NAME="oidc"
...
CONTAINER_MOUNTS="HOST/config/rauthy/config.cfg:/app/rauthy.cfg"
CONTAINER_MOUNTS+=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/oidc.script.sh
__custom_docker_script() {
  cat <<EOF | tee -p | grep -v '^$'
ENV_ENV_ENC_KEYS="${ENV_ENV_ENC_KEYS:-$(openssl rand -base64 32)}"
ENV_ENC_KEY_ACTIVE="${ENV_ENC_KEY_ACTIVE:-$(openssl rand -hex 4)}"
...
if [ -n "$OIDC_USERNAME" ]; then
  CONTAINER_USER_NAME="$OIDC_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${OIDC_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$OIDC_PASSWORD" ]; then
  CONTAINER_USER_PASS="$OIDC_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  ...
  CONTAINER_USER_PASS="${OIDC_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
